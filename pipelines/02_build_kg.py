...
def norm_go_node_id(go_id: str) -> str:
    s = _s(go_id)
    if not s:
        return ""
    s = s.strip()

    # normalize any double-prefix variant
    if s.startswith("go:go:GO:"):
        return "go:" + s.split("go:go:", 1)[1]  # -> go:GO:...

    # already canonical
    if s.startswith("go:GO:"):
        return s

    # mapping may have go:GO: but with weird casing/spacing
    if s.lower().startswith("go:go:"):
        return "go:" + s.split(":", 2)[-1]
...
def norm_theme_id(theme: str) -> str:
    theme = str(theme).strip()
    # remove accidental double tox prefix
    if theme.lower().startswith("tox:tox:"):
        theme = "tox:" + theme.split("tox:tox:", 1)[1]
    if not theme.lower().startswith("tox:"):
        return "tox:" + theme
    return theme
